...
- **Each branch writes `type` itself** (`JobView.type()`, `PendingJobView.type()`), which is why the
  ...
  written, so every row of `GET .../job` went out undiscriminated while `GET .../job/{id}` carried the
  id. A client that trusted the document's discriminator read `undefined` on every listing row. A branch
  that answers the property itself carries it whatever the writer was handed, `Page<JobView>` included.
...
hours is one day rather than a window to scan. Its last 24 buckets are `hourlyLast24h`, which stays
because `completedLast24h` is summed from it.
...
- **OpenAPI**: `EndpointOASFilter` runs at build time to augment the OpenAPI document with inferred metadata:
  ...
  - **Discriminator branches** (`describeBranches`): a `oneOf` names a property its branches never
    declared, and a generator believes it — `JobView` was published with a required `type` literal its
    own schema did not carry. Each branch of a discriminated union now publishes the property with the
    constant it sends (`enum: ["job"]`) and states it `required`. Runs after `describeShapes`, because
    the property has no field behind it and is appended to the `required` list that pass produced.
