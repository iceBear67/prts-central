...
accepts is settled by `MockWorkerE2ETest`, which is the only place the two meet.

```
worker-mock/
  src/main/java/io/ib67/prts/worker/mock/
    MockWorker.java        the worker: connection, the reply queue, the job roster
...
The control plane answers every message a worker sends with **exactly one message, in order**, and
without a request id: a `result` for anything it accepted or refused, or a `presignedUpload` for an
`uploadArtifactRequest` it accepted. Everything else a worker receives — `createJob`, `cancelJob`,
...
- `MockWorkerE2ETest` in the control plane (`./gradlew e2eTest`, tagged `e2e`) is where the mock meets
