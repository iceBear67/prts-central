...
- **A worker to run jobs on (`worker-mock`)**: a separate subproject holding a worker that speaks the
  ...
  with it. See [worker-mock/README.md](../worker-mock/README.md) and `MockWorkerEntityE2ETest`.
...
- **An agent message is acknowledged before it is handled.** `AgentAttached`, `AgentFrame` and `AgentDetached` get `Ack(true, "")` once dispatched, and `AgentService` handles them afterwards on the event bus. Wait for the channel (`AgentChannels.get(jobId)`) before connecting a viewer. A refusal reaches only the log; since one worker's messages are handled in order, it can be checked once a later message from the same worker has taken effect (`AgentWebSocketE2ETest.aWorkerCannotAnnounceAnAgentForAJobItDoesNotHost`).
