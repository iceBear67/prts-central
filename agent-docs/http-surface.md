...
- **User references**: Where a view names a user it does not describe (`JobView.requestedBy`, `TaskView.createdBy`, `SubAccountView.createdBy`), it carries a `UserInfo` `{id, name}` rather than a bare UUID. The referenced rows hold no foreign key and outlive the user, so `name` is `null` when the account has since been deleted — a gap, not an error. Views that are themselves *about* the user (`CurrentUserView`, `UserView`, `ProjectMemberView`) keep their own flat fields. A view factory resolves the user it names itself — the resource passes entities, never a `Map<UUID, User>`. Each such view carries two factories: a single-entity one (`JobView.of(job, createRequest)`, `UserInfo.of(id)`) and a collection one (`JobView.of(jobs, createRequest)`, `TaskView.of(tasks)`) that resolves the whole page in one `User.mapByIds` — a listing must call the collection overload rather than mapping over the single one.
