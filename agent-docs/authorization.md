...
| **Worker Auth** | `X-Worker-Token` on `/ws/worker` | Dedicated | Validates header against `worker.secret`. Every worker authenticates as one shared identity with role `worker`. |
